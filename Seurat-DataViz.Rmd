...
```{r}
table(seurat.object$integrated_snn_res.0.5)
```
...
```
```{r}
propeller(clusters = Idents(seurat.object), sample = seurat.object$orig.ident, group = seurat.object$Obesity, transform = "logit", trend = FALSE, robust = FALSE)
```
```{r}
