...
library(patchwork)
library(cowplot)
rnaProject <- "Obesity_scRNA-SCTRegression"
...
```{r}
seurat.object <- readRDS("Obesity_scRNA-SCTRegression.RDS")
...
```
```{r}
DimPlot(seurat.object, cols = color.palette, group.by = "integrated_snn_res.0.5", split.by = "Obesity", ncol = 3)
```
```{r}
library(ArchR)
